...
def index():
    ...
    # Detail Global (dioptimasi: 8 query jadi 1 GROUP BY)
    detail_grouped = conn.execute("""
        SELECT 
            CASE 
                WHEN procurement_method IN ('E-Purchasing', 'Pengadaan Langsung', 'Dikecualikan') 
                THEN procurement_method 
                ELSE 'Lainnya' 
            END as method_group,
            COUNT(*) as total,
            SUM(CAST(REPLACE(REPLACE(budget, 'Rp ', ''), ',', '') AS REAL)) as total_budget
        FROM procurement 
    ...
    for row in detail_grouped:
        ...
        if g == 'E-Purchasing':
            total_epurchasing = cnt; budget_epurchasing = bgt
        elif g == 'Pengadaan Langsung':
            total_pl = cnt; budget_pl = bgt
        elif g == 'Dikecualikan':
            total_dikecualikan = cnt; budget_dikecualikan = bgt
        else:
            total_lainnya = cnt; budget_lainnya = bgt
    
    # Realisasi Global (dioptimasi: 6 query jadi 1 GROUP BY + 1 total)
    ...
    real_grouped = conn.execute("""
        SELECT 
            CASE 
                WHEN "Metode Pengadaan" IN ('E-Purchasing', 'Pengadaan Langsung', 'Dikecualikan') 
                THEN "Metode Pengadaan" 
                ELSE 'Lainnya' 
            END as method_group,
            SUM("Total Nilai (Rp)") as total_budget
        FROM realisasi 
        GROUP BY method_group
    """).fetchall()
    ...
    for row in real_grouped:
        ...
        if g == 'E-Purchasing':
            real_epurchasing = bgt
        elif g == 'Pengadaan Langsung':
            real_pl = bgt
        elif g == 'Dikecualikan':
            real_dikecualikan = bgt
        else:
            real_lainnya = bgt
    ...
    # Detail Filtered (dioptimasi: 8 query jadi 1 GROUP BY)
    f_detail_grouped = conn.execute(f"""
        SELECT 
            CASE 
                WHEN procurement_method IN ('E-Purchasing', 'Pengadaan Langsung', 'Dikecualikan') 
                THEN procurement_method 
        ...
        GROUP BY method_group
    """, dynamic_params).fetchall()
    
    f_total_epurchasing = 0; f_budget_epurchasing = 0
    f_total_pl = 0; f_budget_pl = 0
    f_total_dikecualikan = 0; f_budget_dikecualikan = 0
    f_total_lainnya = 0; f_budget_lainnya = 0
    for row in f_detail_grouped:
        g, cnt, bgt = row['method_group'], row['total'], row['total_budget'] or 0
        if g == 'E-Purchasing':
            f_total_epurchasing = cnt; f_budget_epurchasing = bgt
        elif g == 'Pengadaan Langsung':
            f_total_pl = cnt; f_budget_pl = bgt
        elif g == 'Dikecualikan':
            f_total_dikecualikan = cnt; f_budget_dikecualikan = bgt
        else:
            f_total_lainnya = cnt; f_budget_lainnya = bgt
    ...
    # Hitung persentase realisasi tiap paket
    for pkg in packages:
        budget_str = pkg.get('budget', 'Rp 0')
        budget_num = float(budget_str.replace('Rp ', '').replace(',', '')) if budget_str else 0
        pkg_real_total = pkg.get('realisasi_total', 0) or 0
        pkg['has_realisasi'] = pkg_real_total > 0
        pkg['realisasi_persen'] = round((pkg_real_total / budget_num) * 100, 1) if budget_num > 0 and pkg_real_total > 0 else 0
