...
def generate_signal():
    ...
    reverse_candidate = weakest_curr + strongest_curr
    ...
    for active_pair, info in list(open_positions.items()):
        ...
        position_updates.append(f"📌 {active_pair} status: {status} (B:{b_pow:.1f}/Q:{q_pow:.1f})")

    if strong_val >= 7.0 and weak_val <= 2.0:
        action = "🟢 STRONG BUY ⬆️" if is_strongest_base else "🔴 STRONG SELL ⬇️"
        ...
        for j in range(i + 1, len(CURRENCIES)):
            ...
            for p in PAIRS:
                if c1 in p and c2 in p:
                    actual_pair = p
                    break
            if not actual_pair: actual_pair = c1 + "/" + c2
            
            base = actual_pair.split('/')[0] if '/' in actual_pair else actual_pair[:3]
            quote = actual_pair.split('/')[1] if '/' in actual_pair else actual_pair[3:]
            
            action = "🟢 STRONG BUY ⬆️" if all_powers[base] > all_powers[quote] else "🔴 STRONG SELL ⬇️"
            if gap < 5.0: action = "⚪ NEUTRAL ↔️"
            
            decision = "ENTER NOW" if gap >= 7.0 else "WAIT FOR DIP" if gap >= 5.0 else "AVOID"
            conf_emoji = "✅" if gap >= 5.0 else "⏳"
            
            all_gaps.append({
                'pair': actual_pair, 'gap': gap, 'strong': c1 if p1 > p2 else c2,
    ...
    for idx, s in enumerate(top_3, 1):
        msg_parts.append(f"\n{idx}. {s['pair']}")
        msg_parts.append(f"⚡ Power: {s['strong']} ({s['strong_val']:.2f}) vs {s['weak']} ({s['weak_val']:.2f})")
        msg_parts.append(f"📈 Action: {s['action']}")
        msg_parts.append(f"🛠 Conf: H1 {s['conf']} | M15 {s['conf']} | M5 {s['conf']}")
        msg_parts.append(f"🎯 Decision: {s['decision']}")
